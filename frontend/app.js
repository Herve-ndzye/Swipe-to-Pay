...
socket.on('connect', () => {
  ...
  // Load initial stats and check health
  loadStats();
  checkBackendHealth();
});
...
topupBtn.addEventListener('click', async () => {
  ...
  try {
    const requestBody = {
      uid: lastScannedUid,
      amount: currentMode === 'remove' ? -amount : amount  // Negative for remove mode
    };
...
async function checkBackendHealth() {
  try {
    ...
    if (response.ok) {
      ...
      updateSystemStatus('backend', health.backend === 'online');
      updateSystemStatus('mqtt', health.mqtt === 'connected');
      updateSystemStatus('db', health.database === 'connected');
    } else {
      updateSystemStatus('db', false);
    }
  } catch (err) {
...
addModeBtn.addEventListener('click', () => {
  ...
  // Update UI text
  transactionTitle.innerHTML = '💵 Load Payment Card';
  amountLabel.textContent = 'Load Amount ($)';
  amountIcon.textContent = '💰';
  actionText.textContent = 'Load Card Balance';
  actionIcon.textContent = '✓';
  
  // Update button style
  topupButton.classList.remove('remove-mode');
});
...
removeModeBtn.addEventListener('click', () => {
  ...
  // Update UI text
  transactionTitle.innerHTML = '💸 Remove Money';
  amountLabel.textContent = 'Remove Amount ($)';
  amountIcon.textContent = '💸';
  actionText.textContent = 'Remove from Balance';
  actionIcon.textContent = '✓';
  
  // Update button style
  topupButton.classList.add('remove-mode');
});

// Update quick amount button clicks to work with current mode
